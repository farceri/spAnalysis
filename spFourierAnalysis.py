...
def computeStructureFactor(dirName, plot="plot"):
    ...
    sfList = utils.getStructureFactor(pos, qList, numParticles)
    if(plot == "plot"):
...
def averageStructureFactor(dirName, dirSpacing=1):
    ...
    for dir in dirList:
        pos = utils.getPBCPositions(dirName + os.sep + dir + "/particlePos.dat", boxSize)/scale
        sf += utils.getStructureFactor(pos, qList, numParticles)
...
def averageVelocityStructureFactor(dirName, dirSpacing=1):
    ...
    for dir in dirList:
        ...
        velsf += utils.getVelocityStructureFactor(pos, vel, qList, numParticles)
...
def averageSpaceFourierEnergy(dirName, dirSpacing=1):
    ...
    for dir in dirList:
        ...
        kqtemp, uqtemp, kctemp = utils.getSpaceFourierEnergy(pos, vel, epot, qList, numParticles)
        energyq[:,0] += kqtemp
        energyq[:,1] += uqtemp
        energyq[:,2] += kctemp
...
def averageTimeFourierEnergy(dirName, dirSpacing=1):
    ...
    energyTransform = utils.getTimeFourierEnergy(dirName, dirList, dirSpacing, numParticles)
...
def averageSpaceFourierVelocity(dirName, dirSpacing=1):
    ...
    for dir in dirList:
        pos = utils.getPBCPositions(dirName + os.sep + dir + "/particlePos.dat", boxSize)/scale
        vel = np.array(np.loadtxt(dirName + os.sep + dir + "/particleVel.dat"), dtype=np.float64)
        vf += utils.getSpaceFourierVelocity(pos, vel, qList, numParticles)
...
def averageTimeFourierVel(dirName, dirSpacing=1):
    ...
    dirList = dirList[np.argwhere(timeList%dirSpacing==0)[:,0]]
    timeList = timeList[np.argwhere(timeList%dirSpacing==0)[:,0]]
    velTransform = utils.getTimeFourierVel(dirName, dirList, dirSpacing, numParticles)
